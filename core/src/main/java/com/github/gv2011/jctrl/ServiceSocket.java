...
public final class ServiceSocket implements AutoCloseableNt{
  ...
  private static final Logger LOG = getLogger(ServiceSocket.class);
  ...
  public static ServiceSocket create(final AutoCloseableNt controlled, final String processName) {
    boolean success = false;
    final ServerSocket serverSocket = call(() -> new ServerSocket());
    try {
      call(() -> serverSocket.bind(new InetSocketAddress(InetAddress.getLoopbackAddress(), SERVICE_PORT)));
      final ServiceSocket result = new ServiceSocket(serverSocket, controlled, processName);
      success = true;
      return result;
    } finally {
  ...
  private ServiceSocket(final ServerSocket serverSocket, final AutoCloseableNt controlled, final String processName) {
